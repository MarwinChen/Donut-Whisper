...
srt_root = "/hdd/srt11/data_8844/Mediastorm/SRT_Mediastorm"
video_root = "/hdd/srt11/data_8844/Chinese_video"
output_dir = "/hdd/srt11/data_8844/Cut_chinese"

# srt_root = "/hdd/srt11/data_8844/Manshi/SRT_Manshi"
# video_root = "/hdd/srt11/data_8844/Manshi/Manshi_video"
# output_dir = "/hdd/srt11/data_8844/Cut_chinese_manshi"


output_json = "/hdd/srt11/data_8844/Train_zh.json"

all_srt = {}
srt_files = os.listdir(srt_root)
video_files = os.listdir(video_root)
deleteFile = ['4B8vf-yP3XU', 'g_cU-DrUPGI', 'glG-5L1aMuQ', 'H1xhhzqoZMY', 'k6_6GI8mhnA', 'm3FIMRakvHA', 'q2Tim1cqhQo', 'tXejzvNuwDw']
for srt_file in srt_files:
    # if  srt_file[0] == '_' or srt_file[0] == '0' or srt_file[0] == '1': # Test
    # if  srt_file[0] == '2' or srt_file[0] == '3' or srt_file[0] == '4' or srt_file[0] == '5': # Val
    if not (srt_file[0] == '_' or srt_file[0] == '0' or srt_file[0] == '1' or srt_file[0] == '2' or srt_file[0] == '3' or srt_file[0] == '4' or srt_file[0] == '5' or srt_file[:-4] in deleteFile ): # Train
        with open(os.path.join(srt_root, srt_file), 'r') as file:
            ...
            for entry in entries:
                ...
                if len(lines) >= 3:
                    ...
                    match_zh = lines[2]
                    if match_zh:
                        subtitle_zh = match_zh
                        result.append((start_time, end_time, subtitle_zh))
...
for k, v in all_srt.items():
    # video_lst += [os.path.join(video_root, f'The.Good.Wife.{k.split(".")[0]}.2009.1080p.WEB-DL.x265.10bit.AC3-BitsTV.mkv')] * len(v)
    video_lst += [os.path.join(video_root, f'{k.split(".")[0]}.mp4')] * len(v)
    time_lst += v
res = []
...
def process(i):
    ...
    output_audio_path = os.path.join(output_dir, "audio", video_name, f"{video_name}-{round(start_time)}-{round(end_time)}.opus")
